...
describe("VolcengineChatLanguageModel", () => {
        ...
        prompt: [
          {
            role: "user",
            content: [
              { type: "text", text: "Compare weather in Beijing and Shanghai" }
            ]
          }
        ]
      });
  ...
  describe("doStream", () => {
    it("should stream text response", async () => {
      ...
      expect(
        parts.some((p: any) => p.type === "text-delta" && p.delta === " world")
      ).toBe(true);
    ...
    it("should stream reasoning content", async () => {
      ...
      expect(
        parts.some(
          (p: any) => p.type === "reasoning-delta" && p.delta === "Thinking..."
        )
      ).toBe(true);
    ...
    it("should stream tool calls", async () => {
      ...
      expect(
        parts.some(
          (p: any) => p.type === "tool-call" && p.toolName === "get_weather"
        )
      ).toBe(true);
    });
  });
